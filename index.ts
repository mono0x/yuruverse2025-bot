...
const takeScreenshot = async (page: Page, rule: Rule, name: string): Promise<string> => {
...
const post = async (page: Page) => {
    ...
    if (appKey == null || appSecret == null || accessToken == null || accessSecret == null) {
    ...
    if (!((mediaIds: string[]): mediaIds is [string, string] => {
        return mediaIds.length == 2;
    })(mediaIds)) {
        throw new Error("Too many media files, Twitter allows a maximum of 4 media per tweet.");
    }
